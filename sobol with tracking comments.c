...
//Attempting to dissect the sobol functionality...
...
//main function designed to be fed through the commend line meh...
main(argc,argv)
int argc;
char **argv;
{
	...
	//loop through the entire 256x256 pixel image
        ...
        //assign new pixel values for output, essentially drawing the 'soboled' lines here that passed
        //the threshold test
        for (i=0;i<256;i++)
          { for (j=0;j<256;j++)
            {
